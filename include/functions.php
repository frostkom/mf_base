...
function render_image_tag($data)
{
	...
	if($data['id'] > 0)
	{
		...
		$image_tag = preg_replace('/(width|height)="1"\s?/', '', $image_tag); // This will make sure that SVG files does not have height="1" width="1"
		$out .= $image_tag;
	}
...
function get_post_types_for_select($data = [])
{
	...
	do_log(__FUNCTION__.": Still in use from Base");
